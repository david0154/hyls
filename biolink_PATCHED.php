<?php
// biolink_PATCHED.php - ADDS CROP + VIDEO to existing biolink.php (SAFE FOR LIVE)
...
try {
    ...
    $video_table_name = '';
    try {
        // Try bio_social_videos first (new table)
        $stmt = $db->query("SHOW TABLES LIKE 'bio_social_videos'");
        if ($stmt->rowCount() > 0) {
            $has_video_table = true;
            $video_table_name = 'bio_social_videos';
        }
    } catch (Exception $e) {
        error_log("Video table check error: " . $e->getMessage());
    }
    ...
    try {
        if ($has_crop_columns) {
            ...
            $stmt = $db->prepare("SELECT id, user_id, image_url, image_order, created_at FROM bio_gallery WHERE user_id = ? ORDER BY image_order ASC LIMIT 6");
        }
        $stmt->execute([$user_id]);
    ...
    // Get social videos (if table exists)
    $social_videos = [];
    if ($has_video_table) {
        try {
            // Use user_id directly instead of bio_profile_id
            ...
            // If user_id column doesn't exist, try with bio_profile_id
            try {
                $stmt = $db->prepare("SELECT * FROM bio_social_videos WHERE bio_profile_id = (SELECT id FROM bio_profiles WHERE user_id = ?) ORDER BY display_order ASC");
                $stmt->execute([$user_id]);
                $social_videos = $stmt->fetchAll(PDO::FETCH_ASSOC);
            } catch (Exception $e2) {
                error_log("Social videos error: " . $e2->getMessage());
            }
        }
    }
    ...
    // Handle gallery upload (UPDATED FOR 12MB + CROP)
    ...
    // MAIN FORM SUBMISSION (Keep existing code exactly as is)
...
?>
<!-- HTML in next message due to length -->